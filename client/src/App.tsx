...
const JsonFormatter = ({ data }: { data: any }) => {
  if (
    ...
    return (
      <span className="timestamp-val" data-iso={iso}>
        {data}
      </span>
    );
  }
  ...
  if (typeof data === "number")
    return <span style={{ color: "#fb923c" }}>{data}</span>;
...
function App() {
  ...
  const clearSession = () => {
    if (
      window.confirm(
        "Are you sure you want to clear the current session? Unsaved changes will be lost.",
      )
    ) {
      setSessionName("");
      setMatchRules("");
      ...
      try {
        ...
        if (Array.isArray(imported)) {
          ...
          const isValid = imported.every(
            (item) => item.id && item.matchRules && item.samples,
          );
          if (isValid) {
  ...
  const importDatadogIntegrations = (
      ...
      try {
        ...
        if (Array.isArray(integrations)) {
          ...
          if (newSessions.length > 0) {
            ...
            showToast(
              `Imported ${newSessions.length} grok processors from integrations`,
            );
          } else {
  ...
  const exportAsTerraform = () => {
    const rulesList = matchRules
      ...
      .map((line: string) => {
        ...
        const name =
          firstSpaceIdx !== -1 ? line.substring(0, firstSpaceIdx) : "rule";
        const pattern =
          firstSpaceIdx !== -1 ? line.substring(firstSpaceIdx).trim() : line;
        return `    {
            ...
            <div style={{ display: "grid", gap: "1rem" }}>
              ...
              <div>
                <label className="label-text">
                  Match Rules (RULE_NAME PATTERN, one per line)
                </label>
                <textarea
                  value={matchRules}
              ...
              <div>
                <label className="label-text">
                  Support Rules (Optional, e.g. RULE_NAME PATTERN)
                </label>
                <textarea
                  value={supportRules}
              ...
              <div key={sample.id} className="sample-row">
                ...
                {results[sample.id] && sample.text.trim() && (
                  <div
                    ...
                    style={{ opacity: results[sample.id].isLoading ? 0.6 : 1 }}
                  >
          ...
          <div className="card">
            <div className="section-title">
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                ...
                <button
                  className="btn btn-outline"
                  onClick={() => ddIntegrationInputRef.current?.click()}
                >
                  ...
                  <div className="history-info">
                    ...
                    <div className="history-summary">
                      ...
                      <div style={{ color: "var(--text-muted)" }}>
                        ...
                        {item.matchRules.split("\n").length > 1 ? " ..." : ""}
                        <span style={{ margin: "0 8px", color: "#e2e8f0" }}>
                          |
                        </span>
